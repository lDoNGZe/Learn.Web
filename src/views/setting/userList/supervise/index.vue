    ...
    >
      ...
      <template v-slot:operation="slotScope">
        <div class="btn-container">
          <el-button
            v-auth="['sprUser.btn.edit']"
            type="text"
            @click="handleEdit(slotScope.data)"
          >编辑</el-button>
          <el-button
            v-auth="['sprUser.btn.delete']"
            type="text"
...
export default {
  ...
  data() {
    ...
    return {
      ...
      tableConfig: {
        ...
        // 是否展示导出
        hasExportBtn: true,
        exportBtnCodeList: ["sprUser.btn.export"],
        hasSelect: true,
        // 表格是否展示loading
        loading: true,
        // 导出数据文件名称
  ...
  methods: {
    ...
    handleDetail(rowData) {
      this.$router.push(
        `/profession/supervision/record/labourer/detail?id=${rowData.id}`
      );
    },
    handleUserConfirm() {
    ...
    async getEnterpriseByEprType(keyword) {
      const { code, result } = await ListEnterprises({
        eprName: keyword,
        eprType: 1,
      });
      if (code === 200) {
    ...
    async getRoles() {
      const { code, result } = await getRoles({ roleType: 0 });
      if (code === 200) {
